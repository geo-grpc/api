...
public class TestBuffer extends TestCase {
    ...
    public void testBufferPolytonWKT() {
        ...
        com.esri.core.geometry.Geometry buffered = OperatorBufferEx.local().execute(geom, sr, 2.0, null);
    ...
    public void testBufferPoint() {
        ...
        OperatorBufferEx buffer = (OperatorBufferEx) OperatorFactoryLocalEx.getInstance().getOperator(OperatorEx.Type.Buffer);
        ...
        boolean is_simple = simplify.isSimpleAsFeature(result, sr.toSpatialReference(), true, nsr,
                null);
    ...
    public void testBufferEnvelope() {
        ...
        NonSimpleResult nsr = new NonSimpleResult();
        assertTrue(simplify.isSimpleAsFeature(result, sr.toSpatialReference(), true, nsr, null));
    ...
    public void testBufferPolygon() {
        ...
        {
            Geometry result = buffer.execute(inputGeom, sr.toSpatialReference(), 0, null);
            assertTrue(result.getType().value() == Geometry.GeometryType.Polygon);
            assertTrue(result == inputGeom);
        }

        {
            Geometry result = buffer.execute(inputGeom, sr.toSpatialReference(), 10, null);
            assertTrue(result.getType().value() == Geometry.GeometryType.Polygon);
            Polygon poly = (Polygon) (result);
            Envelope2D env2D = new Envelope2D();
            result.queryEnvelope2D(env2D);
        ...
        {
            sr = SpatialReferenceEx.create(4326);
...

        {
            ...
            Geometry result = buffer.execute(inputGeom, sr.toSpatialReference(), -2, null);
            assertTrue(result.getType().value() == Geometry.GeometryType.Polygon);
            Polygon poly = (Polygon) (result);
            Envelope2D env2D = new Envelope2D();
            result.queryEnvelope2D(env2D);
