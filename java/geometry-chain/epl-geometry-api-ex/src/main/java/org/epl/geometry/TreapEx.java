...
final class TreapEx {
    static abstract class Comparator {
        ...
        abstract int compare(TreapEx treap, int elm, int node);
        ...
        void onDeleteImpl_(TreapEx treap, int node) {
            if (m_b_notify_on_actions)
                onDelete(treap.getElement(node));
        }

        void onSetImpl_(TreapEx treap, int node) {
            if (m_b_notify_on_actions)
    ...
    static abstract class MonikerComparator {
        ...
        abstract int compare(TreapEx treap, int node);
    }
    ...
    // Returns next treap index. Allows to navigate TreapEx in the sorted order
    public int getNext(int treap_node_index) {
        return m_treapData.getField(treap_node_index, 6);
    }

    // Returns prev treap index. Allows to navigate TreapEx in the sorted order
    // backwards
    ...
    // Returns the maximum depth of this TreapEx at given moment
    ...
    private TreapEx.Comparator m_comparator;// comparator used to arrange the
    // nodes
